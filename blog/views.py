...
class PostAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if title:
            return Post.objects.filter(titles__name__icontains=title)
        else:
...
class DetailAPIView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        ...
        if category:
            return Post.objects.filter(categories__name__icontains=category)
        if author:
            return Post.objects.filter(authors__name__icontains=author)
        if comments:
            return Post.objects.filter(comments__name__icontains=comments)
        else:
            return Post.objects.all()
